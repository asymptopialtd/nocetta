 ...
 */
export function check(nodes, repoState, locator) {
    const byId = new Map(nodes.map((n) => [n.id, n]));
    const dirty = new Set();
    const reasons = new Map();
    const markDirty = (id, reason) => {
        if (!dirty.has(id)) {
            dirty.add(id);
    ...
    while (queue.length > 0) {
        ...
        if (currentNode && isSuperseded(currentNode))
            continue; // wall: do not propagate past a superseded node
...
export function currentNodes(nodes, repoState, locator) {

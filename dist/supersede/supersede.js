...
function wouldCycle(nodes, fromId, toId) {
 ...
 */
export function supersede(nodes, oldId, next, opts = {}) {
    const old = nodes.find((n) => n.id === oldId);
    if (!old)
        throw new Error(`supersede: unknown node "${oldId}"`);
    ...
    const newEdges = [...next.edges, { type: "supersedes", target: oldId }];
    if (opts.restates)
        newEdges.push({ type: "restates", target: opts.restates });
    return {

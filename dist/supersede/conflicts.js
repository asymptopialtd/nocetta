...
function documentFrequency(parties) {
    const df = new Map();
 ...
 */
function discriminatingTokens(node, df, total) {
...
export function directlyRelated(a, b) {
    return (a.edges.some((e) => RELATION_TYPES.has(e.type) && e.target === b.id) ||
...
export function findConflicts(nodes) {
    ...
    for (const [subject, group] of groups) {
        ...
        const sameSubject = (a, b) => anchored || intersects(disc.get(a.id), disc.get(b.id));
        const candidates = [];

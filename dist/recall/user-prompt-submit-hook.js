 ...
 */
export function runUserPromptSubmitHook(repoRoot, input) {
    if (isOff())
        return { output: null };
    ...
    const additionalContext = `nocetta: a current belief that may relate to this — "${summary}" (id ${pick.node.id.slice(0, 8)}); ` +
 ...
 */
export function priorInjectionsFor(repoRoot, sessionId) {
    if (!sessionId)
        return [];
    ...
    for (const e of events) {
        if (e.kind === "compaction" && (e.session === sessionId || e.session === undefined) && e.t > resetAt)
            resetAt = e.t;
    }
    const lastInjectById = new Map();
    for (const event of events) {
        if (event.kind !== "inject" || event.session !== sessionId)
            continue;
        if (resetAt && event.t <= resetAt)
            continue; // pre-compaction — expired
 ...
 */
export function extractFilePaths(prompt, repoRoot, cwd) {
    const out = new Set();

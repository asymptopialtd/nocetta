 ...
 */
export const TOOLS = [
    defineTool("memory_search", "The recall surface: call it BEFORE editing code to pull up what memory knows about the files you are about to touch, or to answer a question from stored facts. " +
        ...
        if (used_ids && used_ids.length > 0)
            appendRecall(nc.repoRoot, { t: now, kind: "ack", ids: used_ids });
        if ((files_in_play?.length ?? 0) === 0 && (keyword?.trim().length ?? 0) === 0) {
            ...
            throw new Error("memory_search: give files_in_play (repo-relative paths you are about to edit) or keyword — with neither the only honest answer is an empty one, so the call refuses");
        }
    ...
    defineTool("memory_remember", "Capture a durable fact so a future session recalls it here. kind is \"claim\" (a fact about code), \"value\" (a decision or convention), \"entity\" (a thing and its aliases) or \"lore-fact\" (a fact about docs/plans). " +
        ...
        if (body.length > VERBOSE_BODY_CHARS) {
            ...
            notes.push(`body is ${body.length} chars — consider a lead sentence then **Why:**/**How to apply:** sections (with [[wikilinks]] to related nodes), or supersede this node with a tighter one`);
        }
    ...
    defineTool("memory_supersede", "Correct a belief: writes a new node that supersedes old_id — the old node is never deleted, it stays in history, and recall resolves to the new one. " +
    ...
    defineTool("memory_worklist", "What needs attention: dirty nodes (the code each is anchored to changed since capture — listed with the reason verbatim and the artifactPath to repair against), cross-authority conflicts that need a supersede, duplicate pairs (two live beliefs saying the same thing — converge them by superseding the weaker copy), and quarantined store files. " +
        "Run it after edits or refactors, and before trusting recall. Dirty nodes are repaired via memory_repair (action \"reanchor\"), or retired there if the belief is simply gone. Example: {}.", {}, (nc) => shapeWorklist(nc.worklist(), nc.issues())),
    defineTool("memory_repair", "Close an item from memory_worklist. action \"reanchor\": the dirty node's anchor is re-resolved against the artifact's current source and its locator+hash rewritten — the belief itself is untouched; give symbol_name (the exact symbol name) or heading (the exact heading), the same names memory_remember accepts. " +
    ...
    defineTool("memory_as_of", "Time travel: the beliefs the store held at transaction time at (ISO 8601) — including facts that have since been superseded or retired. " +

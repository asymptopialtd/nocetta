 ...
 */
function receiverTypeName(method) {
 ...
 */
export function extractGoSymbols(filePath, source) {
        ...
        switch (node.type) {
            ...
            case "type_declaration": {
                ...
                for (const spec of node.namedChildren) {
                    ...
                    if (name)
                        addSymbol(spec, [`type ${name}`], "class", name);
                }
                break;
            }
                ...
                for (const spec of node.namedChildren) {
                    ...
                    for (const nameNode of spec.childrenForFieldName("name")) {
                        if (nameNode?.type === "identifier")
                            addSymbol(spec, [`const ${nameNode.text}`], "const", nameNode.text);
                    }
                }
                break;
            }

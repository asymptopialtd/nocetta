 ...
 */
export function filenameFor(node) {
    const slug = node.body
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "")
        .slice(0, 48)
 ...
 */
function withPosixArtifactPaths(node) {
    return {
        ...node,
        anchors: node.anchors.map((anchor) => anchor.artifactPath.includes("\\") ? { ...anchor, artifactPath: anchor.artifactPath.replaceAll("\\", "/") } : anchor),
    };
}
/**
 ...
 */
export function writeNode(dir, node) {
    const violations = scanForSecrets(node);
    if (violations.length > 0)
        throw new NeverLeakError(node.id, violations);
    if (!existsSync(dir))
    ...
    for (const name of names) {
        try {
            ...
            if (firstProblem === undefined)
                ...
                issues.push({ file: name, reason: firstProblem });
        }
        catch (err) {
 ...
 */
export function buildReverseIndex(nodes) {
    const index = new Map();
    for (const node of nodes) {
        for (const anchor of node.anchors) {
            const ids = index.get(anchor.artifactPath) ?? new Set();
            ids.add(node.id);

...
const GDScript = require("tree-sitter-gdscript") as Parser.Language;
 ...
 */
export function extractGdscriptSymbols(filePath: string, source: string): SymbolInfo[] {
  ...
  for (const node of tree.rootNode.namedChildren) {
    switch (node.type) {
      ...
      case "class_name_statement": {
        ...
        if (name) addSymbol(node, [`class ${name}`], "class", name);
        break;
      }
      ...
      case "const_statement": {
        const name = nameOf(node);
        if (name) addSymbol(node, [`const ${name}`], "const", name);
        break;
      }
        ...
        if (body) {
          for (const member of body.namedChildren) {
            if (member.type === "function_definition") {
              ...
              if (methodName) addSymbol(member, [...prefix, `method ${methodName}`], "method", methodName);
            } else if (member.type === "const_statement") {
              const constName = nameOf(member);
              if (constName) addSymbol(member, [...prefix, `const ${constName}`], "const", constName);
            }
          }
        }
        break;

 ...
 * `tree-sitter-go.wasm`, so no native build is needed. The one-time async init
 * runs at module load via top-level await, keeping {@link extractGoSymbols}
 * synchronous for callers, matching symbols.ts.
 */
const require = createRequire(import.meta.url);
await Parser.init({
  locateFile: () => require.resolve("web-tree-sitter/tree-sitter.wasm"),
});
const language = await Language.load(require.resolve("tree-sitter-go/tree-sitter-go.wasm"));

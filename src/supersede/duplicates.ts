...
function containment(
  ...
  for (const token of small) {
    ...
    minSide += weight;
    if (big.has(token)) intersection += weight;
  }
  return minSide > 0 ? intersection / minSide : 0;
}

function dfOver(tokenSets: readonly Set<string>[]): Map<string, number> {
...
function idfFromDf(df: Map<string, number>, total: number): Map<string, number> {
 ...
 */
export function findDuplicates(nodes: MemoryNode[]): Duplicate[] {
  ...
  const headlineSets = headline.filter((s): s is Set<string> => s !== null);
  ...
  for (let i = 0; i < live.length; i++) {
    for (let j = i + 1; j < live.length; j++) {
      ...
      const score = Math.max(
        containment(full[i]!, full[j]!, fullIdf),
        headline[i] !== null && headline[j] !== null
          ? containment(headline[i]!, headline[j]!, headlineIdf)
          : 0,
      );
      const threshold = sharedAnchors.length > 0 ? ANCHORED_THRESHOLD : CROSS_THRESHOLD;
      if (score >= threshold) duplicates.push({ a, b, score, sharedAnchors });
    }
  }

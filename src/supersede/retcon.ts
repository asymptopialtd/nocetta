 ...
 */
export function retconImpact(nodes: MemoryNode[], supersededId: string): string[] {
  return nodes.filter((n) => n.edges.some((e) => e.type === "anchored-to" && e.target === supersededId)).map((n) => n.id);
}

...
The loop:
- BEFORE editing a file, call memory_search with files_in_play — memories anchored to the files you are about to touch come back ranked and current-only. Use keyword for decision-shaped questions ("why is retry capped at 3"). After you act on what it returns, your next memory_search can name the ids you used in used_ids — a light acknowledgement that tells nocetta which memories earn their keep.
- AFTER finishing a task or refactor, call memory_worklist: dirty memories (the code each describes changed), cross-authority conflicts and duplicate pairs needing a supersede, and unreadable store files. Close every item — memory_repair with action "reanchor" when the belief is still true but the code moved, action "retire" (reason required) when the belief's subject is gone, memory_supersede to converge a duplicate pair (keep the better-anchored, better-worded belief as the tip).
...
Store what git cannot tell the next session: decisions, rationale, behavior facts, project vocabulary — the things a future session would otherwise re-derive or violate silently. Under-capture is the expensive error: a stale memory surfaces in the worklist to be repaired, but a lost decision is gone. Skip transient task state and what a diff already shows; the write gate refuses secrets. On contradiction, supersede (memory_supersede) — never write a conflicting duplicate; when remember returns a duplicate advisory the store already holds the belief, so supersede the named node instead of keeping both. History keeps superseded beliefs and memory_as_of can revisit them.

...
export interface StopHookInput {
  turn_tool_calls?: Array<{ tool_name?: string; tool_input?: Record<string, unknown> }>;
  cwd?: string;
}
 ...
 * the agent's context.
 */
export function runStopHook(repoRoot: string, input: StopHookInput): StopHookResult {
  const editedFiles = editedFilesOf(input, repoRoot);
  if (editedFiles.size === 0) return { acked: [] };

  const surfaced = surfacedIds(repoRoot);
  if (surfaced.size === 0) return { acked: [] };
  ...
  if (acked.length > 0) appendRecall(repoRoot, { t: new Date().toISOString(), kind: "ack", ids: acked });
  return { acked };
}
...
function editedFilesOf(input: StopHookInput, repoRoot: string): Set<string> {
  const cwd = input.cwd ?? repoRoot;
  const out = new Set<string>();
  for (const call of input.turn_tool_calls ?? []) {
    if (call.tool_name === undefined || !EDIT_TOOLS.has(call.tool_name)) continue;
...
function surfacedIds(repoRoot: string): Set<string> {
  const ids = new Set<string>();
  for (const event of readRecallLog(repoRoot)) {
    if (event.kind === "recall") for (const id of event.ids) ids.add(id);
  }
  return ids;
}

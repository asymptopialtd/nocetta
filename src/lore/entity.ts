...
function namesOf(entity: MemoryNode): string[] {
  return [entity.body, ...(entity.aliases ?? [])];
}

function matchesName(entity: MemoryNode, nameOrAlias: string): boolean {
 ...
 */
export function findReferences(nodes: MemoryNode[], entityId: string): MemoryNode[] {
  const entity = nodes.find((n) => n.id === entityId);
  if (!entity || entity.kind !== "entity") throw new Error(`findReferences: unknown entity "${entityId}"`);
...
export function goToDefinition(nodes: MemoryNode[], nameOrAlias: string): MemoryNode | undefined {
...
export function renameEntity(
  ...
  if (!old) throw new Error(`renameEntity: unknown node "${entityId}"`);
  if (old.kind !== "entity") throw new Error(`renameEntity: "${entityId}" is not an entity`);

  const carriedOverAliases = [old.body, ...(old.aliases ?? [])];
  const withAliases: MemoryNode = {
    ...next,
    kind: "entity",
